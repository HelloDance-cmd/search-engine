...
class GraspByKeyword(Grasp):
    ...
    @staticmethod
    def add_to_mysql(page_info_wrapper: List[Tuple[str, str, str]], k):
        ...
        # 添加到words表中
        if cursor.execute("SELECT id FROM words WHERE words=%s", (k,)) == 0:
            cursor.execute("INSERT INTO words(words) VALUE (%s)", (k,))
            mysql.commit()
            cursor.execute("SELECT LAST_INSERT_ID()")
        else:
            cursor.execute("SELECT id FROM words WHERE words=%s", (k,))

        words_id = cursor.fetchone()[0]
        # 添加到tag表中


        for official_name, url, title, text in page_info_wrapper:
            # 添加到url表中
            cursor.execute("INSERT INTO url(official_name, url, words_id) VALUE (%s, %s, %s)",
                           (official_name, url, int(words_id)))
            mysql.commit()

            # 添加到text表中
            cursor.execute("SELECT LAST_INSERT_ID()")
            url_id = cursor.fetchone()[0]
            cursor.execute("INSERT INTO text(url_id, words_id, title, text) VALUE (%s, %s, %s, %s)",
                           (int(url_id), int(words_id), title, text))
            mysql.commit()
